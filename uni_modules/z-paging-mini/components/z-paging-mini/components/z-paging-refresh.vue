	...
	<view style="height: 100%;">
		<view :class="showUpdateTime?'zp-r-container zp-r-container-padding':'zp-r-container'">
			<view class="zp-r-left">
				<image v-if="status!==R.Loading" :class="leftImageClass" :style="[leftImageStyle]" :src="leftImageSrc" />
				<!-- #ifndef APP-NVUE -->
				<image v-else class="zp-line-loading-image zp-r-left-image" :style="[leftImageStyle]" :src="leftImageSrc" />
				<!-- #endif -->
				<!-- #ifdef APP-NVUE -->
				<view v-else :style="[{'margin-right':showUpdateTime?'18rpx':'12rpx'}]">
					...
					:style="[{color:zTheme.indicator[ts]}]" :animating="true" />
				</view>
				<!-- #endif -->
			</view>
			<view class="zp-r-right">
				<text class="zp-r-right-text" :style="[rightTextStyle]">{{currentTitle}}</text>
				<text v-if="showUpdateTime&&refresherTimeText.length" class="zp-r-right-text zp-r-right-time-text" :style="[rightTextStyle]">
					{{refresherTimeText}}
				</text>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		props: ['status', 'defaultThemeStyle', 'defaultText', 'pullingText', 'refreshingText', 'completeText', 'completeImg', 'showUpdateTime', 'updateTimeKey', 'updateTimeTextMap'],
		computed: {
			ts() {
