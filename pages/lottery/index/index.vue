...
export default {
  ...
  methods: {
    ...
    // 跨年判定：若最近一条记录的 createTime 年份不等于今年，则视为跨年，
    // 改以"今年第一期 (D1, C1)"为锚点重新计算，避免拿去年记录跨年累加导致期号错乱。
    ...
    // 计算并返回今日七星彩的期号，七星彩只有每周二五日有
    // 跨年判定：若最近一条记录的 createTime 年份不等于今年，则视为跨年，
    // 改以"今年第一期 (D1, C1)"为锚点重新计算；否则沿用按最近记录累加的逻辑。
    // 能够循环向前查询，防止因为断买了的日期无法续上期号
