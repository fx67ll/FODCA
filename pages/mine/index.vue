<template>
  <view class="mine-container" :style="{ height: `${windowHeight}px` }">
    ...
    <view class="content-section">
      ...
      <view class="menu-list">
        <view class="list-cell list-cell-arrow" @click="handleToCubeGame" v-if="userName && userName !== 'fx67ll'">
          <view class="menu-item-box">
...
import { getSecretConfigBatch } from "@/api/fx67ll/secret/key.js";
...
export default {
  ...
  data() {
    return {
      ...
      // 是否需要打卡工时的配置项（仅对 fx67ll 生效，控制「打卡工时管理」入口显隐）
      isNeedPunchIn: uni.getStorageSync("isNeedPunchIn") || false,
      // 最新公告弹窗
  ...
  onShow() {
    ...
    this.isNeedPunchIn = uni.getStorageSync("isNeedPunchIn");
  ...
  methods: {
    ...
    // 跳转服务状态大盘（公开页，免登录）
    // 公共逻辑抽离至 @/utils/server/status.js，供设置页复用
    ...
    getWKConfig() {
      const self = this;
      // 批量查询非敏感功能开关（一次请求替代多次）
      getSecretConfigBatch(["isNeedWaiKuai", "isNeedPunchIn"]).then((res) => {
        if (res && res?.rows && res?.code === 200) {
          res.rows.forEach((item) => {
            const val = JSON.parse(item.secretValue || "false");
            if (item.secretKey === "isNeedWaiKuai") {
              uni.setStorageSync("isNeedWaiKuai", val);
              self.isNeedWaiKuai = val;
            } else if (item.secretKey === "isNeedPunchIn") {
              uni.setStorageSync("isNeedPunchIn", val);
              self.isNeedPunchIn = val;
            }
          });
        }
      });
