<template>
  <view class="fx67ll-tools-box">
    <scroll-view scroll-y="true" class="fx67ll-tools-scroll">
      <view class="fx67ll-ocr-lottery">
        <view class="fx67ll-drawer-title" v-if="!ocrTagList || ocrTagList.length < 1">测试页，后期看看能填充什么内容</view>
        ...
        <view class="fx67ll-ocr-result" v-if="ocrTagList && ocrTagList.length > 0">
          <span v-for="(item, index) in ocrTagList" :key="item.timeStamp"
            :class="{ 'fx67ll-ocr-tag': true, 'fx67ll-ocr-tag-check': item.isCheck }" @click="changeTagStatus(index)">{{
              item.words || "--" }}
          </span>
        </view>
      </view>
